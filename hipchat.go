...
type HipchatItem struct {
	Message *HipchatMessage `json:"message"`
...
func talkHandler(w http.ResponseWriter, r *vRequest) {
	var reply string
	message := stripPrefix(r.HCRequest.GetRoomMessage())
	if message == "" {
